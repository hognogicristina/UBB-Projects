...
void AdminGUI::initAdminGUI() {
	...
	gridLayout->addWidget(this->undoButton, 1, 1);
	undoButton->setStyleSheet(
			"background-color: antiquewhite; font-size: 15px; height: 25px; width: 120px; border: 1px solid black;");

	gridLayout->addWidget(this->redoButton, 1, 2);
...
UserGUI::UserGUI(QWidget *parent, Service &service1, UserService &userService1, TrenchCoatValidator &validator1,
	...
	this->openTable = new QPushButton("Open table");
	...
	this->shoppingBasketTableModel = new TableView{this->userService};
	this->initUserGUI();
...
void UserGUI::initUserGUI() {
	...
	radioButtonsLayout->addWidget(this->openTable, 1, 1);
	openTable->setStyleSheet(
			"background-color: antiquewhite; font-size: 15px; height: 25px; width: 120px; border: 1px solid black;");
...
void UserGUI::createTable() {
	...
	tableView->setModel(this->shoppingBasketTableModel);
...
void UserGUI::connectSignalsAndSlots() {
	...
	QObject::connect(this->openTable, &QPushButton::clicked, [this]() {
		this->createTable();
	});
...
void UserGUI::populateShoppingBasketTable() {
	this->shoppingBasketTableModel->propagate();
}
...
void UserGUI::addTrenchCoat() {
	...
	if (!this->repoTypeSelected) {
		...
		try {
			...
			this->populateShoppingBasketTable();
