...
const CategoriesScreen = () => {
    ...
    const handleWebSocketMessage = (data) => {
        try {
            ...
            Toast.show({
                ...
                text2: `Item: ${item.name}`,
                visibilityTime: 5000,
                onPress: () => navigation.navigate('ItemsList', {category: item.category})
            });
        } catch (error) {
    ...
    const loadCategories = async () => {
        ...
        try {
            ...
            appendLog('Error fetching categories from server');
            if (storedCategories) {
                setCategories(storedCategories);
            ...
            {isOffline && !categories.length ? (
                ...
                <>
                    <TouchableOpacity
                        ...
                        onPress={() => navigation.navigate('DiscountedItems')}
                        disabled={isOffline}
                    >
                    ...
                    <FlatList
                        ...
                        renderItem={({item}) => (
                            <TouchableOpacity
                                ...
                                onPress={() => navigation.navigate('ItemsList', {category: item})}
                            >
