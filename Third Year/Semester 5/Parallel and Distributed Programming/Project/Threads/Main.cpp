...
int main() {
    ...
    // Example 1: Larger Graph
    Graph graph1 = Graph::generateRandomGraph(20, 20); // Create a random graph with 50 nodes and 50 edges

    Color colors1(4); // Increase the number of colors
    colors1.addColor(0, "red");
    colors1.addColor(1, "blue");
    colors1.addColor(2, "green");
    colors1.addColor(3, "yellow");

    std::vector<int> partialCodes(graph1.getNodesNumber(), -1); // Initialize partial codes

    std::mutex lock;
    std::vector<int> codes;

    int specificColorNumber = 3;
    ...
    Graph graph3 = Graph::generateRandomGraph(70, 70); // Create a random graph with 10 nodes and 15 edges
